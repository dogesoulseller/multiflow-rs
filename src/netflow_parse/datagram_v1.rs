...
impl NetflowDatagramV1Record {
	pub fn parse_from_datagram(input: &[u8]) -> IResult<&[u8], NetflowDatagramV1Record> {
		...
		Ok((res, NetflowDatagramV1Record {
			src_ip,
			dst_ip,
			next_hop_ip,
			snmp_in_if_idx,
...
impl NetflowDatagramV1 {
	pub fn parse_from_datagram(input: &[u8]) -> IResult<&[u8], NetflowDatagramV1> {
		...
		Ok((res, NetflowDatagramV1 { sys_uptime_ms, unix_sec, unix_nsec, flow_records }))
	}
}